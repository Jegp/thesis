...
x_j = \sigma(u_j) = \sigma([\sum_{i=1}^n w_{i,j} x_i] + b_j)
\end{equation}
...
This is the equivalent of summing up $n$ neuronal events in a trial,
...
\begin{equation} \label{eq:dirac}
  \rho(t) = \int dt\ \delta(t) = \sum_{i=1}^n \delta(t - t_i) = 1
\end{equation}
...
\caption{A model of how a constant, low input current produces a buildup of
	voltage, a spike and finally a refractory period
         of an integrate-and-fire \index{neuron model!integrate-and-fire} 
         neuron.
	 No voltage is integrated during the refractory period.}
\label{fig:spiking}
\end{figure}
...
spike train\index{spike train}, and rate models simply counts the timestamps and divides
them by the duration of the spike train to produce the spike \textit{rate}, shown
in Equation \ref{eq:spikerate} \cite{Dayan2001, Eliasmith2004}.
Semantically this is the equivalent of the average number of spikes propagated
...
\begin{equation}
  f(I) = (\tau_{ref} - \tau_{RC}\; log(1 - {V_{th} \over IR}))^{-1}
  \label{eq:lif-rate}
\end{equation}

Assuming that the stochastic process smoothes out over time, this equation
relates input in \gls{ANN}s to input in \gls{SNN}s.
